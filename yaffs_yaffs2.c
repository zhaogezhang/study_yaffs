...
/*********************************************************************************************************
** 函数名称: yaffs_calc_oldest_dirty_seq
** 功能描述: 从指定设备中找出时间最久远的数据块，更新设备的 oldest_dirty_seq 和 oldest_dirty_block 字段值
** 输	 入: dev - yaffs 设备
** 输	 出: 
** 全局变量: 
** 调用模块: 
...
/*********************************************************************************************************
** 函数名称: yaffs2_find_oldest_dirty_seq
** 功能描述: 如果当前设备不知道最久远的数据块号，则计算
** 输	 入: dev - yaffs 设备
** 输	 出: 
** 全局变量: 
** 调用模块: 
...
/*********************************************************************************************************
** 函数名称: yaffs_block_ok_for_gc
** 功能描述: 校验指定的块是否比当前设备已知的、最久远的块更加久远（更加久远表示更加适合用于垃圾回收）
**         : 如果传入的块更加久远，则更新设备的 oldest_dirty_seq 和 oldest_dirty_block 字段值
** 输	 入: dev - yaffs 设备
**         : bi - 需要校验的块信息
** 输	 出: 1 - 我们传入的块比当前设备已知的更加久远
**         : 0 - 我们传入的块没有当前设备已知的久远
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: yaffs2_find_refresh_block
** 功能描述: 查找当前设备中有效的、时间最久远的块号，但是并不是每次调用这个函数都会执行查找操作
**         : 需满足如下条件才会查找：
**         : 1. 当前设备使用的是 yaffs2
**         : 2. 查找周期不小于 10（dev->param.refresh_period >= 10）
**         : 3. 查找周期变量已经递减到 0（变化趋势是从 dev->refresh_skip 递减，直到为 0 重新加载值）
** 输	 入: dev - yaffs 设备
** 输	 出: oldest - 找到的有效且最久远的块号
**         : 0 - 不满足查找条件
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: yaffs2_checkpt_required
** 功能描述: 计算在当前设备上我们是否需要开启 checkpt 功能，开启条件分别是：
**         : 1. 使用的是 yaffs2
**         : 2. 不“跳过”写 checkpt 数据，且设备不是只读设备
**         : 3. 当前设备空间大小满足最小要求（即空间块数 >= YAFFS_CHECKPOINT_MIN_BLOCKS）
** 输 	 入: dev - yaffs 设备
** 输 	 出: 
** 全局变量: 
** 调用模块: 
...
/*********************************************************************************************************
** 函数名称: yaffs_calc_checkpt_blocks_required
** 功能描述: 计算指定设备存储 checkpt 数据需要多少个块
** 输 	 入: dev - yaffs 设备
** 输 	 出: retval - 存储 checkpt 数据需要的块数
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
...
/*********************************************************************************************************
** 函数名称: yaffs2_handle_hole
** 功能描述: 对指定的文件添加“空洞”信息，如果空洞空间比较小，则直接往里面填充 0，如果空洞空间比较大
**         : 我们通过更新文件头信息，并且设置头信息的 shrink 属性实现
** 输     入: obj - 需要处理的对象指针
**         : new_size - 处理后文件大小
** 输     出: YAFFS_OK - 执行成功
**         : YAFFS_FAIL - 执行失败
