...
/*********************************************************************************************************
** 函数名称: yaffs_load_tags_to_spare
** 功能描述:	 计算 tags（8字节）的 ecc 并放到 tags->ecc 字段，然后根据 yaffs1 文件系统把 tags 信息按照约
**         : 定格式放到 spare（16字节）中，在需要把 tags 信息往 flash 中写入时调用
** 输     入: dev - 设备指针
**         : spare_ptr - spare 源数据指针
**         : tags_ptr - tags 目的数据指针
** 输     出: 
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
				     ...
				     struct yaffs_tags *tags_ptr)
{
	...
	/*
	 * 计算 tags 数据的 ecc 并放到 tags->ecc 字段位置
	 */
...
/*********************************************************************************************************
** 函数名称: yaffs_get_tags_from_spare
** 功能描述:	 yaffs1 文件系统从 spare（16字节）中提取出 tags（8字节）信息，在从 flash 中读取 spare 后
**         : 要提取 tags 信息时调用
** 输     入: dev - 设备指针
**         : spare_ptr - spare 源数据指针
**         : tags_ptr - tags 目的数据指针
			       ...
			       int correct_errors)
{
	...
	/*
	 * 如果使用 nand 硬件 ecc，读取完数据直接返回，如果使用软件计算 ecc，那么在
	 * 读取完数据之后，还需要计算 ecc
	 */
...
{
	struct yaffs_spare spare; // 16 字节
	struct yaffs_tags tags;   // 8  字节

	yaffs_spare_init(&spare);

	/*
	 * 写入数据有两种操作，一种是写“软删除”标记，一种是写文件数据
	 */
