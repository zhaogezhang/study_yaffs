...
/*
 * spare 结构体代表的是 yaffs1 oob 数据在物理 flash 中存储布局结构
 */
struct yaffs_spare {
	u8 tb0;
	u8 tb1;
...
struct yaffs_dir_var {
	struct list_head children;	/* list of child links */ // 循环链表
...
struct yaffs_obj {
	...
	u32 yst_mode;  /* 因为 FIFO、字符设备、块设备和网络设备的类型都是 YAFFS_OBJECT_TYPE_SPECIAL，所以需要这个字段来表示这个设备类型 */
...
struct yaffs_param {
	...
	u32 n_reserved_blocks;	/* Tuneable so that we can reduce
				 * reserved blocks on NOR and RAM. n_reserved_blocks 必须大于等于 2 */
...
struct yaffs_dev {
	...
	u32 data_bytes_per_chunk;  /* 如果是 inband_tags，那么等于 total_bytes_per_chunk 减去 tags 长度，否则等于 total_bytes_per_chunk */
	...
	u32 internal_start_block;  /* 最小是 1 */
	u32 internal_end_block;
	int block_offset; /* 表示 dev->param.start_block 和 dev->internal_start_block 的差值 */
	int chunk_offset; /* 等于 block_offset * dev->param.chunks_per_block */
	...
	unsigned gc_disable;
	unsigned gc_block_finder; /* 表示当前 gc 从哪个块号开始查找可用于回收的块空间 */
	unsigned gc_dirtiest;     /* 表示当前 gc 准备回收的 nand_block 号*/
	unsigned gc_pages_in_use; /* 表示 gc_dirtiest 指定的块中有多少个 chunk 存储了有效数据*/
	unsigned gc_not_done;  /* 表示在调用 yaffs_find_gc_block 函数时，没有执行块空间回收的次数（在没有满足回收条件的块时，就不会执行回收操作） */
	unsigned gc_block;  /* 记录了当前 gc 处理的数据块号 */
	unsigned gc_chunk;  /* 在 gc 回收一个块数据时，gc_chunk 会记录当前处理的 chunk 在整个块中的偏移 */
	unsigned gc_skip;
	struct yaffs_summary_tags *gc_sum_tags;

	/* Special directories */
	struct yaffs_obj *root_dir;  // yaffs 文件系统根目录（"/yaffs2/"），而不是操作系统根目录（"/"）
	...
	unsigned seq_number;	/* Sequence number of currently
					allocating block */
	unsigned oldest_dirty_seq;    /* 用于记录当前设备有效的、最久远的 block sequence number（yaffs2）*/
	unsigned oldest_dirty_block;  /* 用于记录当前设备有效的、最久远的块号 */
	...
	u32 n_gc_copies;  /* 在 gc 回收空间过程中，复制单个 nand_chunk 数据到 gc 交换空间中的次数 */
	u32 all_gcs;
	u32 passive_gc_count;  /* gc “非紧急”回收的数据块个数 */
	u32 oldest_dirty_gc_count;
	u32 n_gc_blocks;  /* 记录回收块操作总次数（前台和后台） */
	u32 bg_gcs;  /* 记录垃圾回收的后台进程执行回收块操作的次数 */
